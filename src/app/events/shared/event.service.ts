...
export class EventService implements OnDestroy {
  eventsRef: AngularFireList<IEvent>;
  events$:Observable<any>;
  events:IEvent[]
  subEvents:Subscription;

  constructor(private dataStorageService:DataStorageService, private db:AngularFireDatabase){
    this.eventsRef = db.list<IEvent>('/events');
  ...
  getEventId(eventId:string){
    // const searchedEventById = this.events.find(event => event.id === id);
    // const emptyEvent = {} as IEvent;
    // return searchedEventById ? searchedEventById:emptyEvent
     return this.db.object<IEvent>('events/' + eventId).valueChanges()
  }
  ...
  updateEvent(event:IEvent):void{
    // let index = this.events.findIndex((i)=>{i.id === event.id})
    // this.events[index]= event
    this.db.object('/events' + event.key).update(event)
  }

  addSession(eventKey:any, session:ISession){
    session.voters = ['']
    session.id = eventKey
    this.db.list('/events/'+ eventKey + '/sessions/').push(session);
  }

  getSession(event:IEvent){
    let key:any
    let sessions:ISession[]=[]
    for(key in event.sessions)
    if(event.sessions) sessions.push(event.sessions[key])
